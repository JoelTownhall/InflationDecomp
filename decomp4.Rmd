...
### Chunk 6a: CPI Weights and Split
```{r cpi_weights}
# 1. Define 2023 CPI Weights (ABS 6473.0, Table 1 - 17th Series)
# These are the aggregate weights for the CPI Groups.
...
# 2. Define Crosswalk (HFCE Category -> CPI Group)
...
# 3. Calculate HFCE Shares within CPI Groups
# We use the most recent full year of data (e.g., 2023) from our model_data to split the weights.
# For example, if Alcohol is $100 and Tobacco is $50, Alcohol gets 2/3 of the CPI Group weight.

hfce_split_shares <- model_data %>%
  # Filter for 2023 to align with the weighting reference period
  filter(year(join_date) == 2023) %>%
  group_by(category) %>%
  summarise(Annual_CP = sum(CP, na.rm = TRUE), .groups = "drop") %>%
  # Attach CPI Group
  left_join(hfce_cpi_crosswalk, by = "category") %>%
  # Calculate Share within Group
  group_by(CPI_Group) %>%
  mutate(
    Group_Total_CP = sum(Annual_CP, na.rm = TRUE),
    Share_In_Group = Annual_CP / Group_Total_CP
  ) %>%
  ungroup() %>%
  dplyr::select(category, CPI_Group, Share_In_Group)

# 4. Update Mapping Table with Split Weights
mapping_table <- mapping_table %>%
  # Remove old columns if they exist to avoid duplication
  dplyr::select(category, cp_series_id, cvm_series_id) %>%
  left_join(hfce_split_shares, by = "category") %>%
  left_join(cpi_weights_2023, by = "CPI_Group") %>%
  # Calculate the final split weight
  mutate(
    Weight_CPI = Weight_CPI_Group * Share_In_Group
  )

print("Updated Mapping Table with Split CPI Weights:")
print(mapping_table %>% dplyr::select(category, CPI_Group, Weight_CPI_Group, Share_In_Group, Weight_CPI))

final_data <- final_data %>% 
  #dplyr::select(-Weight_CPI) %>% 
...
decomposition_yoy <- decomposition %>%
  ...
  # Attach Headline CPI to calculate the 'Unknown' residual (Headline - (Demand + Supply))
  left_join(cpi_headline, by = "join_date") %>%
  mutate(Unknown_YoY = CPI_YoY - (Demand_YoY + Supply_YoY)) %>%
  # Remove CPI_YoY to avoid duplication when it is joined again in Chunk 8
  dplyr::select(-CPI_YoY)
```
...
# --- Alternative Graph (Unknown/Residual) ---

# Reshape for Alternative Plot (Demand, Supply, Unknown)
# We replace 'Ambiguous' with 'Unknown' (which absorbs Ambiguous + Error)
plot_long_alt <- plot_data %>%
  dplyr::select(join_date, Demand_YoY, Supply_YoY, Unknown_YoY) %>%
  pivot_longer(cols = -join_date, names_to = "Component", values_to = "Value") %>%
  mutate(Component = gsub("_YoY", "", Component))

# Construct the Alternative Plot
p_alt <- ggplot() +
  # Stacked Bars (Unknown in Grey)
  geom_col(data = plot_long_alt, aes(x = join_date, y = Value, fill = Component), width = 90, alpha = 0.9) +
  # Headline Line
  geom_line(data = plot_data %>% filter(!is.na(CPI_YoY)), aes(x = join_date, y = CPI_YoY, color = "Headline CPI"), size = 1.2) +
  # Colors
  scale_fill_manual(values = c("Demand" = "red", "Supply" = "blue", "Unknown" = "grey")) +
  scale_color_manual(values = c("Headline CPI" = "black")) +
  # Annotations
  annotate("rect", xmin = as.Date("2020-03-01"), xmax = as.Date("2021-12-31"), ymin = -Inf, ymax = Inf, alpha = 0.15, fill = "yellow") +
  annotate("text", x = as.Date("2020-09-01"), y = 5, label = "COVID-19", vjust = -0.5, size = 3.5, fontface = "bold") +
  annotate("text", x = last_date, y = as.numeric(last_val), label = paste0(last_val, "%"), hjust = -0.3, vjust = 0.5, fontface = "bold", size = 4) +
  # Labels
  labs(title = "Decomposition of Australian Inflation (Supply vs Demand)", 
       subtitle = "RBA / Shapiro (2022) Methodology (Residual = Unknown)", 
       y = "Year-ended Inflation (%)", x = NULL, caption = "Source: ABS, RBA. 'Unknown' forces sum to Headline.") +
  theme_minimal() + theme(legend.position = "bottom") + 
  scale_x_date(date_breaks = "2 years", date_labels = "%Y")

# Save Alternative Plot
ggsave("outputs/inflation_decomposition_alternative.png", plot = p_alt, width = 10, height = 6, bg = "white")
